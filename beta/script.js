	...
	$('#buildBrowserTable').dataTable( {
	  "ajax": function (data, callback, settings) {
			...
			if (!buildBrowserQueryCursor) {
				...
				query = db.collection("buildTable").where("name", ">", "").endBefore(buildBrowserPreviousQueryCursor).limit(BUILD_BROWSER_PAGE_LIMIT);
			...
			query.get().then((queryRef) => {
				...
				buildBrowserPreviousQueryCursor = isPreviousQuery ? queryRef.docs[0] : !buildBrowserQueryCursor ? queryRef.docs[queryRef.docs.length-1] : buildBrowserQueryCursor;				
